<!DOCTYPE html>
<html lang="{{ app()->getLocale() }}"> {{-- config/app.phpのlocaleの値を取得 --}}
    <head>
        <meta charset="utf-8">
        ...
        {{-- 各ページごとにtitleを入れるために@yieldで空けておく --}}
        <title>@yield('title')</title>
        
        <!-- Scripts -->
        {{-- Laravel標準で用意されているJavascriptを読み込む --}}
        <script src="{{ secure_asset('js/app.js') }}" defer></script> {{-- publicディレクトリのパスを返す関数 secure_asset --}}
    ...
    <body>
        <div id="app">
            ...
            <nav class="navbar navbar-expand-md navbar-dark navbar-laravel">
                <div class="container">
                    <a class="navbar-brand" href="{{ url('/') }}">{{ config('app.name','Laravel') }}</a> {{-- configフォルダのapp.phpの中にあるnameにアクセスするリンク --}}
